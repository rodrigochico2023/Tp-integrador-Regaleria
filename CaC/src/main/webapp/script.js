...
function actualizarCarrito() {
    ...
    const descuentoEnPesosElement = document.getElementById("descuentoEnPesos");
    ...
    carrito.forEach((producto, index) => {
        ...
        item.innerHTML = `
            ...
            <td>
                <button class="boton-mas " onclick="aumentarCantidad(${producto.id})">+</button>
                <span>${producto.cantidad}</span>
                <button class="boton-mas " onclick="disminuirCantidad(${producto.id})">-</button>
            </td>
        `;
    ...
    const descuentoEnPesos = total * descuento;
    ...
    descuentoEnPesosElement.textContent = `$${descuentoEnPesos.toFixed(2)}`;
}
